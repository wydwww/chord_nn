...
if paths.filep('train.t7') and paths.filep('test.t7') then
   ...
   t = {}
   for f in paths.files("/Users/intel/real_asc/") do
      table.insert(t,f)
   end
   -- remove blank names
   table.sort(t)
   ...
   function readfile(file_name)
      local f = assert(io.open("/Users/intel/real_asc/"..file_name, "r"))
      local t = f:read("*all")
      f:close()
      y = mysplit(t)
   ...
   for i = 1, trSize do
      ...
      print('Loading train data '..i..'/'..trSize..' : '..t[i])
      trainData.data[i] = readfile(t[i])
      trainData.labels[i] = classes[mysplit(t[i], '-')[1]]
   ...
   for i = 1, teSize do
      ...
      testData.labels[i] = classes[mysplit(t[i+trSize], '-')[1]]
      -- classes[string.match(t[i+trSize],"%u?%d?%u?%u?%d?%u?%d?")]
